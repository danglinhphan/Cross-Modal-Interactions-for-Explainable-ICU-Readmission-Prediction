...
def main():
    ...
    if hasattr(best_model, 'term_names_') and hasattr(best_model, 'term_importances'):
        ...
        interaction_list_path = os.path.join(base_dir, 'final_interaction_list_v3.txt')
        with open(interaction_list_path, 'w') as f:
            f.write(f"Top Interactions (Model V3 - Interactions=200 - F1=0.88):\n")
            count = 0
            for idx in indices:
    ...
    metrics = {
        ...
        'note': 'Phase 13 Optimized (High Res)'
    }
    ...
    with open(os.path.join(base_dir, 'outputs/ebm_phase13_optimized/final_metrics.json'), 'w') as f:
